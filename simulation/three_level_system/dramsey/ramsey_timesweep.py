...
def play_timetrace_3levels_final(
    time_vec: np.ndarray,
    omega: np.ndarray,
...
D0_vec = np.linspace(2.85e9, 2.89e9, 20)
for D0 in D0_vec:
    splitting = 74e6  # Hz
    ...
    tau_fe_vec = np.linspace(50e-9, 1000e-9, 1000)

    for tau_fe in tau_fe_vec:
        ...
        c_final_states.append(
            play_timetrace_3levels_final(
                time,
                omega,
                Omega_R_eg,
                Omega_R_fg,
...
datadict = {
    "c_final_states": c_final_states,
    "tau_fe_vec": tau_fe_vec,
    "D0_vec": D0_vec,
    "time": time,
    "omega": omega,
